...
聊天区将连续的思考和工具调用合并为一行摘要，正文按实际输出顺序展示。摘要显示工具数量、
当前活动及失败状态；参数、思考全文和工具结果通过 `F3` 打开的独立过程详情窗查看。
同一次调用从参数准备、等待执行到成功、失败或取消始终更新同一条记录。

详情窗左侧按回复列出步骤，右侧显示所选内容。`Tab` 切换焦点，方向键和翻页键导航，
`[` / `]` 切换参数、结果和原始页，`c` 复制完整内容，`Esc` / `F3` 返回。
窄屏通过 `Tab` 在列表和详情页之间切换。工具结果仍完整保存，并原样回送模型。
