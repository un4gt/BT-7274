...
- 消息区溢出时显示滚动条；流式回复显示动画；思考与连续工具操作默认合为单行摘要，F3 独立详情不改变聊天布局。
- 上翻历史时新输出保持阅读位置；详情窗支持窄屏、滚动、参数/结果/原始页切换，并隔离输入焦点。
- 从实际渲染文字定位鼠标坐标，验证工具结果/标签/窄屏返回、滚动后的步骤与会话点击、消息阅读锚点、Unicode 输入定位和拖选，以及弹窗阻止背景点击。
- MCP 列表和向导在窄终端可滚动，凭据字段始终遮蔽。
- BT 首次开场完整播放后才持久化；中断后仍会重播，重复开场允许跳过，失焦暂停。
- 开场隔离按键和粘贴；降落、震屏、面板展开在小窗口和三种主题下安全渲染，完成帧与聊天布局一致。
- 过渡时传感器持续点亮且逐帧移动，移除过渡层前已与真实聊天画面接合；浅色主题不出现背景亮框。
- 侧栏和输入框边框从显现到结束保持最终位置，单调渐显，面板在交接开始后 650ms 内就绪。
- 空对话 BT 在输入（含空格、换行）时从当前姿势淡出、清空草稿后淡入；反向操作亮度连续，完全隐藏后停止额外刷新，失焦或弹窗时暂停。
- 待机动作依次巡视、握拳校准、竖拇指后循环；静止段只预约下一动作，大中小造型双脚固定、传感器唯一，结束时回到原站姿。
- 历史、提示或生成出现时立即清除 BT；关闭选项后淡出，输入光标保持可见。
- `cargo test -p bt-7274 export_titan_preview -- --ignored` 和 `python scripts/render_titan_preview.py`
  可导出真实 UI 的 HTML 动画和 PNG 分镜至 `target/titan-preview`。
- `cargo test -p bt-7274 export_idle_fade_preview -- --ignored` 可导出输入、清空及快速反向的渐变帧。
- `cargo test -p bt-7274 export_idle_actions_preview -- --ignored` 导出完整 UI 中的三种待机动作，省略动作间等待。
