...
首次启动会完整播放约 3.65 秒的降落、震屏、起身、点亮传感器和聊天面板显现动画，
...
机体上线后连续移动、缩放至聊天区域，采用两端减速的平滑曲线，约 0.9 秒完成交接。
聊天面板从出现时就位于最终位置，边框在约半秒内渐显，内容在 0.65 秒内就绪。
关闭空对话机体显示时，机体在移动过程中逐渐淡出。

`show_titan_when_idle` 仅在无聊天历史、无草稿且未生成时显示 BT；输入空格或换行
也会触发约 180ms 淡出，清空草稿后约 260ms 淡入；中途反向操作从当前亮度衔接。
进入 idle 后先等待 5 秒，再按“巡视 → 握拳校准 → 竖拇指”固定轮播，每个动作约 2.4 秒，
动作之间停留 12 秒。输入时冻结当前姿势并淡出，完全隐藏后重置轮播；短暂反向则连续衔接。
静止段直接等待下一动作，不持续刷新；失焦或打开弹窗时暂停，消息或提示出现时立即清除机体。
两项均可在“设置 → 外观”切换并保存。旧的 `show_titan`
