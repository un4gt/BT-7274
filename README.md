...
首次启动完整播放 BT 降落、震屏与聊天面板显现动画（约 3.65 秒）。机体上线后沿平滑曲线
移动、缩放至聊天区域；面板在最终位置渐显，约 0.9 秒内完成交接。
`show_titan_on_startup` 控制之后每次启动是否重播；`show_titan_when_idle` 控制无历史、无草稿
的新对话中是否显示 BT。也可在“设置 → 外观”分别切换。
输入时机体约 180ms 淡出，清空草稿后约 260ms 淡入；快速输入、删除会从当前亮度衔接。
空对话中先停留 5 秒，随后按“巡视 → 握拳校准 → 竖拇指”固定轮播；每个动作约 2.4 秒，
动作之间停留 12 秒。开始输入时从当前姿势淡出，失焦或打开弹窗时暂停。
